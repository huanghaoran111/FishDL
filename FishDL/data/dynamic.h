...
namespace FishDL
{
    ...
    public:
        ...
        virtual const FishDL::Shape<TDataCate::DimNum>& Shape() const = 0;      // 返回形状信息
        virtual bool operator==(const DynamicBase& val) const = 0;              // 判断两个DynamicBase是否相等
        virtual DynamicConstEvalHandle<EvalType> EvalRegister() const = 0;      // 注册求值
    };

    template<typename TInternalData>
    class DynamicWrapper
        : public DynamicBase<typename TInternalData::ElementType,
    ...
    public:
        ...
        const FishDL::Shape<TInternalData::CategoryTag::DimNum>& Shape() const override final
        {
